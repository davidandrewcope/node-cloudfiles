...
Cloudfiles.prototype.getFiles = function (container, download, callback) {
  ...
  function fetchBatch(options, cb) {
  	options.marker = files.length;
  	
  	common.rackspace(options, cb, function (body) {
  	  var fileBatch = JSON.parse(body);
  	  files = files.concat( fileBatch );
  		
  	  if (fileBatch.length === 10000) {//Rackspace API limit
  	    fetchBatch(options, cb);
  	  } else {
  ...
  fetchBatch({ uri: this.storageUrl(container, true), client: this}, function(results){
